...
use crate::{
    common::{BuilderOptions, SchemaBuilderTrait, build_schema, repeated_elements},
    discover::{MoveStructWithModuleId, discover_structs_for_module},
    parse::parse_structs,
};
    ...
    fn discover_structs(&self) -> Result<Vec<MoveStructWithModuleId>> {
        ...
        while let Some((module_id, module)) = modules.pop_first() {
            ...
            if !modules_to_retrieve.is_empty() {
                bail!(
                    "While processing {module_id} references to modules not available \
                    to the builder were found: {modules_to_retrieve:?}. This makes it impossible \
...
mod tests {
    ...
    async fn test_build_schema() -> Result<()> {
        ...
        for name in &[
            ...
            let path = PathBuf::from(format!("../../../aptos-move/framework/{name}"));
            modules.extend(compile_package(path)?);
        }
        modules.extend(compile_package(PathBuf::from(
