...
pub fn parse_structs(
    ...
    for struc in structs {
        ...
        // If there are no fields, push a dummy field to make the generator happy. Note,
...
pub fn move_type_to_field_type(
    ...
    match field_type {
        ...
        MoveType::Struct(struct_tag) => {
            ...
            //   - FixedPoint64
            //   - Aggregator
            let struct_tag = StructTag::try_from(struct_tag)
                ...
                && struct_tag.is_std_option(&CORE_CODE_ADDRESS)
            {
                ...
                // possibly be null.
                let type_tag = struct_tag.type_args.into_iter().next().context(
                ...
                if let TypeRef::NonNull(field_type) = field_type {
                    ...
                    Err(anyhow::anyhow!(
                        "Expected non-null type for Option inner type but got: {field_type:?}. \
        ...
        MoveType::Function { .. } => bail!("Functions cannot appear in structs from storage"),
    }
}
